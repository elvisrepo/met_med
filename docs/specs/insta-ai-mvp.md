...
14. As an invited user, I want an accept-invite page (one-use token + new password) so that I can activate without public signup.
...
Hard paywall (401/server-truncated): no bypass (ToS). Status `fetch_failed` + domain + reason. Manual fallback (dead-last) fields: title, source, body (min 300 chars), published_at (nullable date), main_image_url (required). Source image mandatory: zero candidates → `needs_image` block, generation refused. Text-only pages: noted edge, unlikely per Dori — blocked for now, revisit if hit.
Stage-scoped statuses: `Article.fetch_status: ok|fetch_failed`; `Generation.status: ready|gen_failed|needs_image|in_review|approved|publish_failed`; `Post.status: draft|in_review|queued|posted|failed|token_expired`. Dashboard may show article/generation-stage rows (fetch_failed, gen_failed) alongside Post rows.
...
Frontend: routes /login /accept-invite?token=... /new /review/:id /dashboard /users, React-Query polling, no Redux. Components: UrlForm, CaptionEditor (live count), ImagePreview, CostPill, StatusTable, InviteForm, AcceptInviteForm. Desktop-first responsive, labels + focus + 44px targets. Visual contract: `docs/Insta-AI-MVP-System-Design/screens/` + tokens in `docs/specs/design-tokens.md`; drift resolutions in `docs/specs/opendesign-reconciliation.md`. Caddy serves SPA, proxies /api/*.
...
Auth invite flow: `POST /api/users/invite` (admin) → one-use token (24h) → `POST /api/users/accept {token, password}` → active. Deactivation preserves history; editors get 403 on /users.
